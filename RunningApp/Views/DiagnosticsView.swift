...
struct DiagnosticsView: View {
    ...
    func loadErrorLogs() {
        // Simulating loading error logs
        if let errorLogURL = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first?.appendingPathComponent("error_logs.txt"),
           let errorLogContent = try? String(contentsOf: errorLogURL) {
            errorLogs = errorLogContent.components(separatedBy: "\n").filter { !$0.isEmpty }
        }
    }
}
